...
Discord bot conversations use the same session storage system (`load_session()` / `save_session()`) as web chat, with per-channel session IDs.

### Session ID Format

- **Guild channels:** `discord_{guild_id}_{channel_id}`
- **DM channels:** `discord_dm_{channel_id}`

Session IDs are generated by `build_session_id(guild_id, channel_id)`.

### Session Behavior

- Each Discord channel gets its own persistent session.
- Messages in the same channel continue the same conversation context.
- The bot only responds to messages that mention it.

### Message Flow

1. User mentions the bot.
2. Bot strips the mention with `strip_bot_mention()`.
3. Bot processes the cleaned message.
4. Bot replies using `split_message()` chunks for Discord's 2000 character limit:
   - First chunk: `message.reply(...)`
   - Remaining chunks: `message.channel.send(...)`

### History Loading

Session history is loaded with `load_session()` and mapped with the same forward/return message types as web sessions:

- Non-agent sender -> `forward`
- Current agent sender (`outo` by default) -> `return`

### Time Context

Discord uses the same time awareness behavior as web sessions: if the latest prior agent response is 1+ minute old, a time-context system message is inserted before processing.

### Session Save

Discord saves with the same `save_session()` function and the same message format (`sender`, `content`, `timestamp`, `category`).
